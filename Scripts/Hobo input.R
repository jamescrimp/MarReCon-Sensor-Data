...
hobo_all <- list.files(path = dir.data, recursive = TRUE, pattern = "*.csv$", full.names = TRUE)

#Exclude files that are using the factory calibration for salinity, rather than the field calibration
hobo_files <- hobo_all[!grepl("NOT calibrated CSVs", hobo_all)]
