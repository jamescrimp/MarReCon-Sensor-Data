...
tcm_data <- read.csv(file.path(dir.csv, "tcm_clean.csv"))
tcm_data$time <- as.POSIXct(tcm_data$Time_UTC)
tcm_data$date <- as.Date(tcm_data$Date)
tcm_data <- tcm_data %>% mutate(hour = floor_date(time, "hour"))
...
#Summarize TCM data by hour

tcm_by_hour <- tcm_data %>% group_by(date, site, region, hour) %>% summarize(HourlySpeed = mean(Speed_cm.s, na.rm = TRUE),
                                                                       MeanHead = mean(Heading, na.rm = TRUE)
)
...
tcm_by_hour <- tcm_by_hour %>% filter(HourlySpeed < 40)

g.current.by.site <- tcm_by_hour |>
  dplyr::filter(site == "BCF1") |>
